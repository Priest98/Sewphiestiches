...
function findByImgNum(num) {
    return products.find(p => 
        ...
        p.images.some(img => img.includes(`birthday_outfit_${num}.`))
    );
}

const target = findByImgNum(1);
if (target) {
    [2, 3].forEach(num => {
        const source = findByImgNum(num);
...
console.log("Birthday Outfit 2 and 3 merged into 1.");
