...
function findByImgNum(num) {
    return products.find(p => 
        p.category === "Birthday Outfit" && 
        p.images.some(img => img.includes(`birthday_outfit_${num}.`) || img.includes(`img_`)) &&
        p.name === `Birthday Outfit ${num}`
    );
}

// Special case for img_ filenames which were numbered 15, 16 etc.
function findByName(num) {
    return products.find(p => p.category === "Birthday Outfit" && p.name === `Birthday Outfit ${num}`);
}

const target = findByName(4);
if (target) {
    [16, 15, 10, 5].forEach(num => {
...
console.log("Birthday Outfit 16, 15, 10, 5 merged into 4.");
