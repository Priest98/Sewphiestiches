...
function findByName(num) {
    return products.find(p => p.category === "Reception Dress" && p.name === `Reception Dress ${num}`);
}

const target = findByName(7);
if (target) {
    [12, 11, 8].forEach(num => {
...
console.log("Reception Dress 12, 11, 8 merged into 7.");
