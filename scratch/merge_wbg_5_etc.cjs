...
function findByName(num) {
    return products.find(p => p.category === "Wedding Ball Gown" && p.name === `Wedding Ball Gown ${num}`);
}

const target = findByName(5);
if (target) {
    [7, 8, 9, 11, 12].forEach(num => {
...
console.log("Wedding Ball Gown 12, 11, 9, 8, 7 merged into 5.");
